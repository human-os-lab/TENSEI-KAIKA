...
# 計算
# =====================
def calc_kantei(birthday_str: str):
    try:
    ...
    if not meimei:
        return None, f'対応範囲外の生年月日です（1924年〜2020年）'
...
# =====================
def init_db():
    conn = sqlite3.connect('kantei.db')
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS records (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        created_at TEXT, name TEXT, birthday TEXT,
...
def save_record(name, birthday, comment, result):
    conn = sqlite3.connect('kantei.db')
    c = conn.cursor()
    c.execute('INSERT INTO records (created_at,name,birthday,comment,result) VALUES (?,?,?,?,?)',
...
def get_records():
    ...
    rows = c.fetchall()
    conn.close()
    return [{'created_at': r[0], 'name': r[1], 'birthday': r[2], 'comment': r[3], 'result': r[4]} for r in rows]
